...
class mainfunt0:
    ...
    def take_step2((a,b),dt):
        ...
        da = - (OMEGA_1/2 * b * dt)*1j + GAMMA * dt/2 * b**2
        db = - (OMEGA_1/2 * a * dt)*1j - GAMMA * dt/2 * a**2
...
(a0,b0) = (1.0+0.0j,0.0 + 0.0j)
n = 100; Q = 1000
EUL, TIME = mainfunt0.single_wavepath2((a0,b0), n)#eulpop(n,Q)
ANAL, ANALTIME = mainfunt0.analpop(n)
#R0 =( (ANAL[1:len(EUL)] - EUL[1:])/ANAL[1:len(EUL)] ) *100
...
pyplot.plot( TIME[:len(EUL)], EUL, color='purple', label='RK instant decay')
