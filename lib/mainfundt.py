...
class mainfundt:
    ...
    def analpop(n):
        ...
        for i in range(len(time)):
            waveform[i] = mainfundt.anal_prob(time[i])
        Wavename = "AnalyticSOL_GAMMA" +str(GAMMA) + "_OMEGA" + str(OMEGA_1) +"_N" + str(N) + "_n" + str(n)
        Timename = Wavename + "TIME"
        mainfundt.save("\Analytic Solutions", Wavename, waveform) 
        mainfundt.save("\Analytic Solutions", Timename, time) 
    ...
    def get_next_position1(step, DT, stationarysol):
        ...
        new_step = mainfundt.do_decay1(step, DT, stationarysol)
    ...
    def single_waveform1(DT, stationarysol,number_of_steps):
        ...
        for i in range(1,number_of_steps):
            waveform[i] = mainfundt.get_next_position1(waveform[i-1,1], DT, stationarysol)
    ...
    def sinpop(n,Q):
        ...
        for i in range(number_of_steps):
                stationarysol[i] = mainfundt.f1(i, DT)
    
    # creates Q individual wavepaths 
        for i in range(Q):
            wavepaths[i] = mainfundt.single_waveform1(DT, stationarysol, number_of_steps)
    ...
    # sums up the wavepaths to create a population waveform
        ...
        Timename = Wavename + "TIME"
        mainfundt.save("\Sin Solutions DT", Wavename, average_wavepath) 
        mainfundt.save("\Sin Solutions DT", Timename, time) 
    ...
    def single_wavepath2((a0,b0), n):
        ...
        ab[0] = (a0,b0)
        for i in range(1,number_of_steps): 
            if mainfundt.do_decay2(ab[i-1],dt) == 0: 
                ab[i] = (1.0 + 0.0j, 0.0 + 0.0j)
        else:
                ab[i] = mainfundt.normalise2(mainfundt.take_step2(ab[i-1], dt))
    ...
    def eulpop(n,Q):
        ...
        # creates Q individual wavepaths 
        for i in range(Q):
            wavepaths[i] = mainfundt.single_wavepath_bitch2((a0,b0), number_of_steps, dt)
    ...
    # sums up the wavepaths to create a population waveform
        ...
        Timename = Wavename + "TIME"
        mainfundt.save("\Euler Solutions DT", Wavename, average_wavepath) 
        mainfundt.save("\Euler Solutions DT", Timename, time) 
    ...
    def take_step_rk((a,b),dt):
        ...
        k1a = mainfundt.f_rk(b) ; k1b = mainfundt.f_rk(a)
        k2a = mainfundt.f_rk(b+(dt/2)*k1b) ; k2b = mainfundt.f_rk(a+(dt/2)*k1a)
        k3a = mainfundt.f_rk(b+(dt/2)*k2b) ; k3b = mainfundt.f_rk(a+(dt/2)*k2a)
        k4a = mainfundt.f_rk(b+dt*k3b) ; k4b = mainfundt.f_rk(a+dt*k3a)
    ...
    def single_wavepath_rk((a0,b0), n):
        ...
        for i in range(1,number_of_steps): 
            if mainfundt.do_decay2(ab[i-1],dt) == 0: 
                ...
                ab[i] = mainfundt.normalise2(mainfundt.take_step_rk(ab[i-1], dt))
    ...
    def rkpop(n,Q):
        ...
        # creates Q individual wavepaths 
        for i in range(Q):
            wavepaths[i] = mainfundt.single_wavepath_bitch_rk((a0,b0), number_of_steps, dt)
        ...
        Wavename = "RungeKuttaSOL_GAMMA" +str(GAMMA) + "_OMEGA" + str(OMEGA_1) +"_N" + str(N) + "_n" + str(n)
        Timename = Wavename + "TIME"
        mainfundt.save("\Runge Kutta Solutions DT", Wavename, average_wavepath) 
        mainfundt.save("\Runge Kutta Solutions DT", Timename, time) 
        return average_wavepath, time
    ...
    def compare_Q_sin(n,Qs):
        analwave, analtime = mainfundt.analpop(n)
        msd = numpy.zeros((len(Qs),2))
        msd[:,1] = Qs
        for i in range(len(Qs)):
            numericwave, singlewave, numerictime = mainfundt.sinpop(n,Qs[i])
            msd[i,0] = mainfundt.msd(analwave, numericwave)
        msdname = "SinMSDQs" + str(Qs) + "_GAMMA" +str(GAMMA) + "_OMEGA" + str(OMEGA_1) +"_N" + str(N)
        mainfundt.save("\Sin Solutions DT", msdname, msd) 
        return msd
    ...
    def compare_Q_eul(n,Qs):
        ...
        msd[:,1] = Qs
        for i in range(len(Qs)):
            numericwave, numerictime = mainfundt.eulpop(n,Qs[i])
            msd[i,0] = mainfundt.msd(analwave, numericwave)
        msdname = "EulerMSDQs" + str(Qs) + "_GAMMA" +str(GAMMA) + "_OMEGA" + str(OMEGA_1) +"_N" + str(N)
        mainfundt.save("\Euler Solutions DT", msdname, msd) 
        return msd
    ...
    def compare_Q_rk(n,Qs):
        ...
        msd[:,1] = Qs
        for i in range(len(Qs)):
            numericwave, numerictime = mainfundt.rkpop(n,Qs[i])
            msd[i,0] = mainfundt.msd(analwave, numericwave)
        msdname = "RungeKuttaMSDQs" + str(Qs) + "_GAMMA" +str(GAMMA) + "_OMEGA" + str(OMEGA_1) +"_N" + str(N)
        mainfundt.save("\Runge Kutta Solutions DT", msdname, msd) 
        return msd

    @staticmethod    
    def compare_n_msd3(ns,Q):
        msd = numpy.zeros((len(ns),2))
        msd[:,1] = ns
        for i in range(len(ns)):
            analwave, analtime = mainfundt.analpop(ns[i])
            numericwave, numerictime = mainfundt.rkpop(ns[i],Q)
            msd[i,0] = mainfundt.msd(analwave, numericwave)
        msdname = "RungeKuttaMSDns" + str(ns) + "_GAMMA" +str(GAMMA) + "_OMEGA" + str(OMEGA_1) +"_N" + str(N)
        mainfundt.save("\Runge Kutta Solutions DT", msdname, msd) 
        return msd 
...
if __name__ == "__main__" :
    NS = [10,100]; Q = 100
    q = mainfundt.compare_n_msd3(NS, Q)