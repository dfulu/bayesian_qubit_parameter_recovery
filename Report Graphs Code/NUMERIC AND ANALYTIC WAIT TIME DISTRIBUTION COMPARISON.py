...
N = 10# input('Enter Number of Cycles N (5): ')
...
class mainfunt0:
    ...
    def normaliseRK( (a,b ),dt):
        '''For the case of a Jump event. Normailses the 2 relative probabilities fed in and returns the normalised values'''
        #need to feed in the previous (a,b) co-ords and this assumes that it will decay to the ground state
        scale = ( (abs(a)**2 + abs(b)**2))**0.5        
        a1 = a / scale
        b1 = b / scale
        scale2 = (1-GAMMA*b1**2*dt)**0.5
        a_norm = a1 / scale2
        b_norm = b1 / scale2   
        print '[a1,b1,scale2,a_norm,b_norm] = ', [a1,b1,scale2,a_norm,b_norm]
    ...
    def rkpop(n,Q,RF):
        ...
        mainfunt0.save("\Runge Kutta Solutions V2", Allrksname, wavepaths)
        mainfunt0.save("\Runge Kutta Solutions V2", Waitname, waittimes) 
        mainfunt0.save("\Runge Kutta Solutions V2", Wavename, average_wavepath) 
        mainfunt0.save("\Runge Kutta Solutions V2", Timename, time) 
   ...
   ############################## Distribution Junk can go in here #############################################
    ...
    def FreqCalc(Xmin, Xmax, data, bins, norm):
        ...
        mainfunt0.save("\Runge Kutta Solutions V2", filename, freq)
        mainfunt0.save("\Runge Kutta Solutions V2", centrename, centres)    
        return (centres, freq)
    ...
    def analprob(n, RF):
        dt = 1/n
        T =  N
        times = numpy.arange(0,T,dt)
        prob = numpy.zeros(len(times))
        for i in range(1,len(times)): 
            prob[i] = mainfunt0.prob_Rfreq(times[i],RF)
        return (times, prob)
    ...
    def compare_Q_rk(n,Qs,RF):
        ...
        mainfunt0.save("\Runge Kutta Solutions V2", msdname, msd) 
        return msd
    ...
    def compare_n_rk(ns,Q,RF):
        ...
        mainfunt0.save("\Runge Kutta Solutions V2 T0", msdname, msd) 
        return msd 
    ...
    @staticmethod
    def numericwait(n,RF):
        '''generate the expected wait time distribution'''
        dt = 1/n# time step in units of 1 / (rabi frequency)
        T =  N
        time = numpy.arange(0,T,dt)
        ...
        ab[0] = (1.0 + 0.0j,0.0 + 0.0j)
        for i in range(1,number_of_steps):
            ab[i] = mainfunt0.take_step_rk(ab[i-1], dt,RF)
                                            
        #generate returnable data
        prob_ab = abs(ab*(ab.conjugate()))
    
        return (time, prob_ab[:,1]) # returns only the b values
    ############################# FINISHED NUMERIC WAIT DISTRIBUTION ###############################################
'''savefig('demo.png', transparent=True)'''


n = 400; RF = 5.0
(TIME, NUMWAITDIST) = mainfunt0.numericwait(n, RF)
(ANALTIME, ANALDIST) = mainfunt0.analprob(n, RF)

R0 =( (ANALDIST - NUMWAITDIST) )#/ANALPOP[1:len(RKPOP)] )
...
pyplot.plot( ANALTIME, ANALDIST, color='#FF3399', label='analytic', linewidth = 1)
pyplot.plot( TIME[0::STEP], NUMWAITDIST[0::STEP], color='blue', linestyle = '', marker = '.'),# marker = '.',markersize = 10)

pyplot.ylabel(r'$\omega (\tau)$', fontsize=FS*1.3, labelpad=0*scale)
pyplot.ylim(-padding + min(R0)*10 - 0.1, max(ANALDIST) + padding)
#pyplot.xlim(-0.2, 5.2)
#pyplot.yticks(numpy.arange(0, 1, 0.2), fontsize = FS)
pyplot.xticks(fontsize = FS)

#PLOT THE RESIDUALS
pyplot.plot( TIME, R0*10**9 - 0.1, color='blue')
